...
from networkx.drawing.nx_pydot import write_dot as write_dot2
from networkx.readwrite.gexf import write_gexf as write_gexf
...
#        print('Writing dotfile: ' + OutputFilename)
#        write_dot(graph, './' + OutputFilename)

        # write dot file (alternative format - better for filtering later!)
        OutputFilename = InputFilename + '.dot'
        print('Writing dotfile: ' + OutputFilename)
        write_dot2(graph, './' + OutputFilename)

        # write gexf file (good for importing into Gephi)
        OutputFilename = InputFilename + '.gexf'
        print('Writing GEXF: ' + OutputFilename)
        write_gexf(graph, './' + OutputFilename)
